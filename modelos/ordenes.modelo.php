...
class ModeloOrdenes{
	...
	static public function mdlMostrarOrdenes($tabla, $item, $valor){
		...
		if($item != null){
			...
			$stmt = Conexion::conectar()->prepare("SELECT id, estado, fecha, rut_cliente, nombre_cliente, total_neto, total_impto, total, estado_erp, correlativo_erp, solicitante, fecha_erp FROM $tabla WHERE ltrim(EmplrutF) = $valor and ltrim(solicitante) = $valor ");
			...
			$stmt = Conexion::conectar()->prepare("SELECT * FROM vw_ordenes_encabezado ");
